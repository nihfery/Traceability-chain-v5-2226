...
import { formatDate, getActiveStages, humanStage, humanTeaType, statusClasses, statusText } from "../utils/formatters";
import { useLanguage } from "../contexts/LanguageContext";

export default function BatchTable({ action, batches }) {
            ...
            <Link key={batch.id} to={`/batches/${batch.id}`} className="block p-4 transition hover:bg-[#fbf7ef]">
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-sm text-slate-600">{humanTeaType(batch.teaType, language)}</div>
                </div>
              ...
              <div className="mt-3 grid gap-2 text-xs text-slate-500">
                <div>
                  ...
                  {activeStages.length ? activeStages.map((stage) => humanStage(stage.stageName, language)).join(", ") : "-"}
                </div>
                <div>
            ...
            {batches.map((batch) => {
              ...
              return (
                <tr key={batch.id} className="border-t border-[#eee5d8]">
                  ...
                  <td className="px-5 py-4 text-slate-600">{humanTeaType(batch.teaType, language)}</td>
